...
import sys
sys.path.append("..")
import config, misc, tfutil, dataset
...
def get_activations_fingerprint(images, C_im_with_feature, batch_size=50, verbose=False):
    d0 = images.shape[0]
    if batch_size > d0:
        print("warning: batch size is bigger than the data size. setting batch size to data size")
        batch_size = d0
    n_batches = d0//batch_size
    n_used_imgs = n_batches*batch_size
    pred_arr = np.empty((n_used_imgs, np.prod(C_im_with_feature.output_shapes[1][1:])))
    for i in range(n_batches):
        if verbose:
            print("\rPropagating batch %d/%d" % (i+1, n_batches), end="", flush=True)
        start = i*batch_size
        ...
        batch = images[start:end]
        logit, feature = C_im_with_feature.run(misc.adjust_dynamic_range(batch, [0,255], [-1,1]).astype(np.float32), minibatch_size=batch_size, num_gpus=1, out_dtype=np.float32)
        pred_arr[start:end] = feature.reshape(batch_size, -1)
    if verbose:
        print(" done")
    return pred_arr
#-------------------------------------------------------------------------------
...
class API:
    def __init__(self, num_images, image_shape, image_dtype, minibatch_size, feature_type='inception'):
        self.feature_type = feature_type
        if self.feature_type == 'inception':
        ...
        elif self.feature_type == 'fingerprint':
            training_set = dataset.load_dataset(data_dir=config.data_dir, verbose=False, **config.training_set)
            C_im = misc.load_network_pkl()
            self.C_im_with_feature = tfutil.Network('C_im_with_feature', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, **config.C_im_with_feature)
            self.C_im_with_feature.copy_vars_from(C_im)
    ...
    def feed(self, mode, minibatch):
        if self.feature_type == 'inception':
            ...
            act = get_activations_fingerprint(minibatch, self.C_im_with_feature, batch_size=minibatch.shape[0])
